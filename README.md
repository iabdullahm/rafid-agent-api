...
- **Production requirements:** `LND_REST_URL` (https), `LND_INVOICE_MACAROON` (hex), `L402_ROOT_KEY` (`openssl rand -hex 32`), and `DATABASE_URL` or `L402_DATABASE_URL` for cross-instance single-use enforcement. `loadConfig()` refuses to start if any of these is missing. See `.env.example`.
